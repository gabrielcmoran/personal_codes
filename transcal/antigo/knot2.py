...
class Knot:
    ...
    def get_equation(self):
        if math.floor(self.group) == 1: # group 1
            #T[i+1][0][0], T[i+1][0][1], T[i+1][1][0] = sp.symbols('T[i+1][0][0], T[i+1][0][1], T[i+1][1][0]')
            #self.equation = sp.Eq(((self.delta_x/self.delta_y)*(T[i+1][0][1] - T[i+1][0][0]) + (self.delta_y/self.delta_x)*
            #            (T[i+1][1][0] - T[i+1][0][0]) + q_dot_0*self.delta_x/k) ,
            #            ((1/2)*(self.delta_x*self.delta_y/alpha)*(T[i+1][0][0] - T[i][0][0])/delta_t))
            # criar uma matriz de variaveis ?????/?????
            T_i1_0_0, T_i1_0_1, T_i1_1_0 = sp.symbols('T[i+1][0][0], T[i+1][0][1], T[i+1][1][0]')
            self.equation = sp.Eq(((self.delta_x/self.delta_y)*(T_i1_0_0 - T_i1_0_1) + (self.delta_y/self.delta_x)*
                        (T_i1_1_0 - T_i1_0_0) + q_dot_0*self.delta_x/k) ,
                        ((1/2)*(self.delta_x*self.delta_y/alpha)*(T_i1_0_0 - T[i][0][0])/delta_t))
